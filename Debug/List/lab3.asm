...
_0x3:
	.DB  0x0,0x1,0x1,0x0,0x54
_0x4:
	.DB  0x0,0x2,0x1,0x1,0x54,0x1
_0x5:
	.DB  0x0,0x3,0x1,0x2,0x54,0x2
_0x6:
	.DB  0x0,0x0,0x1,0x3,0x54,0x3
_0x7:
	.DB  0x0,0x1,0x2,0x3
...
; 0000 003F TAB[3]=A3;
	...
	ST   X+,R30
	ST   X,R31
; 0000 0040 // Clock Oscillator division factor: 1
...
; 0000 0051 DDRB=(1<<DDB7) | (1<<DDB6) | (1<<DDB5) | (1<<DDB4) | (1<<DDB3) | (1<<DDB2) | (1<<DDB1) | (1<<DDB0);
	LDI  R30,LOW(255)
	OUT  0x4,R30
; 0000 0052 // State: Bit7=1 Bit6=1 Bit5=1 Bit4=1 Bit3=1 Bit2=1 Bit1=1 Bit0=1
; 0000 0053 PORTB=(1<<PORTB7) | (1<<PORTB6) | (1<<PORTB5) | (1<<PORTB4) | (1<<PORTB3) | (1<<PORTB2) | (1<<PORTB1) | (1<<PORTB0);
	OUT  0x5,R30
; 0000 0054 
...
; 0000 005F PORTD=(1<<PORTD7) | (1<<PORTD6) | (1<<PORTD5) | (1<<PORTD4) | (1<<PORTD3) | (1<<PORTD2) | (1<<PORTD1) | (1<<PORTD0);
	LDI  R30,LOW(255)
	OUT  0xB,R30
; 0000 0060 
; 0000 0061 // Timer/Counter 0 initialization
; 0000 0062 // Clock source: System Clock
; 0000 0063 // Clock value: 9,766 kHz
; 0000 0064 // Mode: Normal top=0xFF
; 0000 0065 // OC0A output: Disconnected
; 0000 0066 // OC0B output: Disconnected
; 0000 0067 // Timer Period: 20,07 ms
; 0000 0068 TCCR0A=(0<<COM0A1) | (0<<COM0A0) | (0<<COM0B1) | (0<<COM0B0) | (0<<WGM01) | (0<<WGM00);
	LDI  R30,LOW(0)
	OUT  0x24,R30
; 0000 0069 TCCR0B=(0<<WGM02) | (1<<CS02) | (0<<CS01) | (1<<CS00);
	LDI  R30,LOW(5)
	OUT  0x25,R30
; 0000 006A TCNT0=0x3C;
	LDI  R30,LOW(60)
	OUT  0x26,R30
...
; 0000 008B ASSR=(0<<EXCLK) | (0<<AS2);
	STS  182,R30
; 0000 008C TCCR2A=(0<<COM2A1) | (0<<COM2A0) | (0<<COM2B1) | (0<<COM2B0) | (0<<WGM21) | (0<<WGM20);
	STS  176,R30
; 0000 008D TCCR2B=(0<<WGM22) | (0<<CS22) | (0<<CS21) | (0<<CS20);
	STS  177,R30
...
; 0000 0093 TIMSK0=(0<<OCIE0B) | (0<<OCIE0A) | (1<<TOIE0);
	LDI  R30,LOW(1)
	STS  110,R30
; 0000 0094 
; 0000 0095 // Timer/Counter 1 Interrupt(s) initialization
; 0000 0096 TIMSK1=(0<<ICIE1) | (0<<OCIE1B) | (0<<OCIE1A) | (0<<TOIE1);
	LDI  R30,LOW(0)
	STS  111,R30
; 0000 0097 
; 0000 0098 // Timer/Counter 2 Interrupt(s) initialization
; 0000 0099 TIMSK2=(0<<OCIE2B) | (0<<OCIE2A) | (0<<TOIE2);
	STS  112,R30
; 0000 009A 
...
; 0000 009D // INT1: Off
; 0000 009E // INT2: Off
; 0000 009F // Interrupt on any change on pins PCINT0-7: Off
; 0000 00A0 // Interrupt on any change on pins PCINT8-15: Off
; 0000 00A1 // Interrupt on any change on pins PCINT16-23: Off
; 0000 00A2 // Interrupt on any change on pins PCINT24-31: Off
; 0000 00A3 EICRA=(0<<ISC21) | (0<<ISC20) | (0<<ISC11) | (0<<ISC10) | (0<<ISC01) | (0<<ISC00);
	STS  105,R30
; 0000 00A4 EIMSK=(0<<INT2) | (0<<INT1) | (0<<INT0);
	OUT  0x1D,R30
; 0000 00A5 PCICR=(0<<PCIE3) | (0<<PCIE2) | (0<<PCIE1) | (0<<PCIE0);
	STS  104,R30
; 0000 00A6 
...
_0x8:
; 0000 00CB {
; 0000 00CC input = PIND & 0x01;//PIND pt intrari
	IN   R30,0x9
	ANDI R30,LOW(0x1)
	MOV  R4,R30
; 0000 00CD index = TAB[Q];
	MOV  R30,R6
	LDI  R26,LOW(_TAB)
	LDI  R27,HIGH(_TAB)
	LDI  R31,0
	...
	LD   R7,X+
	LD   R8,X
; 0000 00CE //facem un while si avem doua conditii de iesire, sa vedem daca inputul introdus de noi este bun si daca am ajuns la terminatorul de sir
; 0000 00CF i = 0;
	CLR  R5
; 0000 00D0 ready = 0;
	CLR  R10
; 0000 00D1 while(!ready)
_0xB:
	TST  R10
	BRNE _0xD
; 0000 00D2 {
; 0000 00D3 if(input == *(index+i))//valoarea de la adresa index+i//verific daca atresa de intrare este egala cu una din valorile mele corect de index
	RCALL SUBOPT_0x0
	LD   R30,Z
	CP   R30,R4
	BRNE _0xE
; 0000 00D4 {
; 0000 00D5 //i ul este offesetul fata de index
; 0000 00D6 //actualizam starea
; 0000 00D7 Q = *(index + i + 1);
	RCALL SUBOPT_0x0
	LDD  R6,Z+1
; 0000 00D8 ready=1;//ca sa iesim din while
	LDI  R30,LOW(1)
	MOV  R10,R30
; 0000 00D9 }
; 0000 00DA else if((*index+i) == 'T')//verificam daca am ajuns la terminatorul de sir, indexul sta pe loc si i ul il incrementam din 2 in 2
	RJMP _0xF
_0xE:
	__GETW2R 7,8
	LD   R26,X
...
; 0000 00DC ready = 1;
	...
	MOV  R10,R30
; 0000 00DD }
; 0000 00DE else     //trecem la urmatoarea valoare de intrare daca nu am ajuns la un input bun sau sa ajungem la termiantorul de sir
	RJMP _0x11
_0x10:
; 0000 00DF i+=2;
	INC  R5
